...
public class ImportInventory extends SvrProcess
{
	...
	// 	Added By Jorge Colmenarez 2015-12-02 
	//	Add Support for Costing
	/**	Update Costing					*/
	...
	protected void prepare()
	{
		...
		for (ProcessInfoParameter element : para) 
		{
			...
			//	Add Support for Costing
				...
				p_UpdateCosting = "Y".equals(element.getParameter());
			else if (name.equals("C_AcctSchema_ID"))
				p_C_AcctSchema_ID = ((BigDecimal)element.getParameter()).intValue();
			else if (name.equals("M_CostType_ID"))
				p_M_CostType_ID = ((BigDecimal)element.getParameter()).intValue();
			else if (name.equals("M_CostElement_ID"))
				p_M_CostElement_ID = ((BigDecimal)element.getParameter()).intValue();
			else if (name.equals("AD_OrgTrx_ID"))
				p_AD_OrgTrx_ID = ((BigDecimal)element.getParameter()).intValue();
			//	End Jorge Colmenarez
			else
			...
			while (rs.next())
			{
				...
				if (line.save())
				{
					...
					if (imp.save()){
						noInsertLine++;	
						//	Added By Jorge Colmenarez 2015-12-02 
						//	Add Support for Update Product cost record if Update costing is enabled
						if (p_UpdateCosting) {
							MCost cost = MCost.get (MProduct.get(getCtx(), imp.getM_Product_ID()), 0
									, acctSchema, p_AD_OrgTrx_ID, p_M_CostElement_ID);
							cost.setCurrentCostPrice( imp.get_ValueAsBigDecimal("CurrentCostPrice"));
							if (cost.save()) {
								// nothing here.
							} else {
