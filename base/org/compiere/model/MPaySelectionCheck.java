...
public final class MPaySelectionCheck extends X_C_PaySelectionCheck
{
	 ...
	 */
	public static int exportToFile (MPaySelectionCheck[] checks, File file, int C_PaySelection_ID)
	{
		...
		try
		{
			...
			noLines++;*/
			  Trx trx = (Trx) null;
			  String sql0 = "select count(*) from c_bankaccount a, c_bank b, c_payselection c where a.c_bank_id = b.c_bank_id and  b.name like '%VENEZUELA%' and a.c_bankaccount_id=c.c_bankaccount_id and c_payselection_id ="+ C_PaySelection_ID;
			  int i_bank = DB.getSQLValue(trx, sql0);
			  String retValue = null;
			  PreparedStatement pstmt = null;  
			  if (i_bank >= 1){
			    String sql = "select 'HEADER  '||lpad(substr(C_payselection_ID,1,8),8,'0')||lpad(substr(C_payselection_ID,1,8),8,'0')||"
				+ "'J300620808'||to_char(sysdate,'dd/mm/yyyy')||to_char(paydate,'dd/mm/yyyy') "
				+ "from c_payselection where c_payselection_id=" + C_PaySelection_ID;
			    try
			    {
				  pstmt = DB.prepareStatement (sql, trx);
				  ResultSet rs = pstmt.executeQuery ();
			    ...
			    fw.write(retValue.toString());
			    fw.write(Env.NL);
			    noLines++;
			    StringBuffer sb = new StringBuffer();
			    sb.append(" DELETE xxtemp_paymentprocess");
			    DB.executeUpdate(sb.toString(), (Trx) null);
			
			    sb = new StringBuffer();
			    sb.append(" INSERT INTO xxtemp_paymentprocess")
                .append(" SELECT c_payselectioncheck_id, 2,'CREDITO '|| LPAD (SUBSTR (c_payselectioncheck_id, 1, 8), 8, '0')|| RPAD (SUBSTR (e.a_ident_dl, 1, 10), 10, ' ')|| RPAD (SUBSTR (d.NAME, 1, 30), 30, ' ')")
                .append(" || '00'|| RPAD (SUBSTR (e.accountno, 1, 20), 20)|| ltrim(REPLACE(TO_CHAR (payamt, '000000000000000D99'),'.',','))|| DECODE (SUBSTR (f.swiftcode, 1, 12), 'BSCHVECA', '10', '00')|| SUBSTR (f.swiftcode, 1, 12)")
                .append(" FROM c_payselectioncheck a,")
                .append(" c_payselection b, c_bankaccount c, c_bpartner d, c_bp_bankaccount e, c_bank f")
                .append(" WHERE a.c_payselection_id = b.c_payselection_id")
                .append(" AND b.c_bankaccount_id = c.c_bankaccount_id")
                .append(" AND a.c_bpartner_id = d.c_bpartner_id")
                .append(" AND a.c_bp_bankaccount_id = e.c_bp_bankaccount_id")
                .append(" AND e.c_bank_id = f.c_bank_id ")
                .append(" AND a.c_payselection_id =") 
                .append(C_PaySelection_ID);
			    DB.executeUpdate(sb.toString(), (Trx) null); 
			
			   sb = new StringBuffer();
			   sb.append(" INSERT INTO xxtemp_paymentprocess")
               .append(" SELECT c_payselectioncheck_id, 1,'DEBITO  '|| LPAD (SUBSTR (c_payselectioncheck_id, 1, 8), 8, '0')")
               .append(" || 'J300620808INSUMOS AGRICOLA CULTIVADORES ASOC '|| TO_CHAR (PAYDATE, 'dd/mm/yyyy')|| '00'")
               .append(" || RPAD (RTRIM (TO_CHAR (c.accountno)), 20)|| ltrim(REPLACE(TO_CHAR (payamt, '000000000000000D99'),'.',','))|| 'VEB40'")
               .append(" FROM c_payselectioncheck a, c_payselection b, c_bankaccount c")
               .append(" WHERE a.c_payselection_id = b.c_payselection_id")
               .append(" AND b.c_bankaccount_id = c.c_bankaccount_id")
               .append(" AND a.c_payselection_id =") 
               .append(C_PaySelection_ID);
			   ...
			   sql = "SELECT   text    FROM xxtemp_paymentprocess ORDER BY c_payselectioncheck_id, seq";
			   try
			   {
			 	pstmt = DB.prepareStatement (sql, trx);
				ResultSet rs = pstmt.executeQuery ();
			    ...
			    sql= " SELECT    'TOTAL   ' " +
		           " || LTRIM (TO_CHAR ((SELECT COUNT (*) " +
                   " FROM c_payselectioncheck b " +
                   " WHERE b.c_payselection_id = a.c_payselection_id),'00000')) " +
                   " || LTRIM (TO_CHAR ((SELECT COUNT (*) " +
                   " FROM c_payselectioncheck b " +
                   " WHERE b.c_payselection_id = a.c_payselection_id),'00000')) " +
                   " || LTRIM (REPLACE(TO_CHAR (totalamt, '000000000000000D99'),'.',',')) " +
                   " FROM c_payselection a " +
				...
				String sql= "SELECT 'J300620808' " +
	            "||RPAD (RTRIM (SUBSTR(TO_CHAR (c.accountno),1,20)), 20)" +
	            "||LTRIM (TO_CHAR ((SELECT COUNT (*) FROM c_payselectioncheck Z WHERE Z.c_payselection_id = a.c_payselection_id),'0000'))" +
	            "||ltrim(REPLACE(TO_CHAR (sum(payamt)*100, '0000000000000'),'.',','))"  +
	            "||TO_CHAR (PAYDATE, 'ddmmyyyy')|| '01' " +
	            " FROM  c_payselectioncheck a, c_payselection b, c_bankaccount c " +
	            " WHERE a.c_payselection_id = b.c_payselection_id " +
             	" AND   b.c_bankaccount_id = c.c_bankaccount_id " +
	            " AND   a.c_payselection_id ="  + C_PaySelection_ID + 
	            " group by a.c_payselection_id,c.accountno, paydate "; 
			  try
			  {
				pstmt = DB.prepareStatement (sql, trx);
				ResultSet rs = pstmt.executeQuery ();
			  ...
			  noLines++;
			  /*
			   * SQL Modificado por: Yamel Senih para adaptar el formato de transferencia electronica al Banco del Exterior
			   * SQL Anterior:
			   * sql= "SELECT RPAD (SUBSTR (d.NAME, 1, 60), 60, ' ')|| ltrim(REPLACE(TO_CHAR (payamt*100, '000000000000'),'.',','))||substr(rpad(nvl(xx_payinvoicelist (a.c_payselection_id, a.c_bpartner_id),' '),120),1,120)" +
			       "|| RPAD (SUBSTR (e.accountno, 2, 3), 3)|| RPAD (nvl(SUBSTR (e.accountno, 1, 20),' '), 20)|| rpad(nvl(substr(A_EMAIL,1,60),' '),60)"+
			       " FROM c_payselectioncheck a, " +
			       " c_payselection b, c_bankaccount c, c_bpartner d, c_bp_bankaccount e, c_bank f " +
			       " WHERE a.c_payselection_id = b.c_payselection_id " +
			       " AND b.c_bankaccount_id = c.c_bankaccount_id " +
			       " AND a.c_bpartner_id = d.c_bpartner_id " +
			       " AND a.c_bp_bankaccount_id = e.c_bp_bankaccount_id " +
			       " AND e.c_bank_id = f.c_bank_id " +
			       " AND a.c_payselection_id =" + C_PaySelection_ID  ;
			   */
			  sql= "SELECT RPAD (SUBSTR (d.NAME, 1, 60), 60, ' ')|| ltrim(REPLACE(TO_CHAR (payamt*100, '000000000000'),'.',','))||substr(rpad(nvl(xx_payinvoicelist (a.c_payselection_id, a.c_bpartner_id),' '),120),1,120)" +
			       "|| RPAD (SUBSTR (e.accountno, 2, 3), 3)|| RPAD (nvl(SUBSTR (e.accountno, 1, 20),' '), 20)|| rpad(nvl(substr(A_EMAIL,1,60),' '),60) || '00000000' || d.value"+
			       " FROM c_payselectioncheck a, " +
			       " c_payselection b, c_bankaccount c, c_bpartner d, c_bp_bankaccount e, c_bank f " +
			       " WHERE a.c_payselection_id = b.c_payselection_id " +
			       " AND b.c_bankaccount_id = c.c_bankaccount_id " +
			...
			if (getC_Payment_ID()!=0){
				...
				if (getC_BP_BankAccount_ID()==0){
					...
					for (int i =0;i<bpas.length;i++)
						if (getPaymentRule().equals(bpas[i].get_ValueAsString("PaymentRule")) &&
