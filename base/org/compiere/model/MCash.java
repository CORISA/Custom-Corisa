...
public class MCash extends X_C_Cash implements DocAction
{
	 ...
	 */
	public MCashTax[] getTaxes(boolean requery) {
		...
		try {
			...
			ResultSet rs = pstmt.executeQuery();
			while (rs.next())
				list.add(new MCashTax(getCtx(), rs, get_Trx()));
			rs.close();
			pstmt.close();
			pstmt = null;
		} catch (Exception e) {
		...
		for (MCashLine line : lines) {
			...
			if (!taxList.contains(taxID)) {
				...
				if (iTax != null) {
					...
					iTax.setIsTaxIncluded(line.isTaxIncluded());
		...
		for (MCashTax iTax : taxes) {
			...
			if (tax.isSummary()) {
				...
				for (MTax cTax : cTaxes) {
					...
					//
					MCashTax newITax = new MCashTax(getCtx(), 0,
							get_Trx());
					newITax.setClientOrg(this);
					newITax.setC_Cash_ID(getC_Cash_ID());
	...
	private boolean reverseIt() 
	{
		...
		if (!MPeriod.isOpen(this.getDateAcct(),MDocBaseType.DOCBASETYPE_CashJournal))
			throw new IllegalStateException("@PeriodClosed@");
		
		//	Reverse Allocations
		MAllocationHdr[] allocations = MAllocationHdr.getOfCash(getCtx(), getC_Cash_ID(),null);
		...
		for (MCashLine cashline : cashlines )
		{
			...
			{
				// Comment By Carlos Parada 2013-10-25 Not Functional
			/*	if (cashline.get_ValueAsInt("C_Payment_ID") == 0)
					throw new IllegalStateException("Cannot reverse payment");
					
				MPayment payment = new MPayment(getCtx(), cashline.get_ValueAsInt("C_Payment_ID"),get_Trx());
		...
		//	Delete Posting
		MFactAcct.delete(Table_ID, getC_Cash_ID(), get_Trx());
