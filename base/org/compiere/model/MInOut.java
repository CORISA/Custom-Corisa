...
public class MInOut extends X_M_InOut implements DocAction
{
	...
	public MInOutLine[] getLines (boolean requery)
	{
		...
		String sql = "SELECT M_InOutLine_ID FROM M_InOutLine WHERE M_InOut_ID=? ORDER BY Line";
		...
		try
		{
			...
			while (rs.next())
				list.add(new MInOutLine(getCtx(), rs.getInt("M_InOutLine_ID"), get_Trx()));
			rs.close();
			rs = null;
			pstmt.close();
			pstmt = null;
				...
				&& (line.getM_AttributeSetInstance_ID() == 0))
		{
			if (inTrx)
			{
				...
				if (asi.save())
				{
					...
					/*04-05-2009. BUG001: 
					 * SE COMENTA LA LINEA DE CODIGO YA QUE CREA NUEVA INSTANCIA 
					 * CADA VEZ QUE SE RECIBE MATERIAL					
	...
	public boolean voidIt()
	{
				...
				|| DOCSTATUS_NotApproved.equals(getDocStatus()) )
		{
			...
			for (MInOutLine line : lines) {
				...
				//2014-01-30 Carlos Parada Subtract Qty Reserver for Original Sales Order
				if (line.getOrig_OrderLine_ID()!=0 && IsReturnFromOutPutProcess()){
					MOrderLine origLine = new MOrderLine(getCtx(), line.getOrig_OrderLine_ID(), get_Trx());
					origLine.setQtyReserved(origLine.getQtyReserved().subtract(line.getQtyEntered()));
					origLine.save(get_Trx());
				}
				//End Carlos Parada
	...
	public boolean reverseCorrectIt(MOrder order)
	{
		...
		//	Deep Copy
		/*
		 * LOCALIZACION VENEZUELA JGMS 01/12/2006 AVANTED PRIME LOC001 SE
		 * COMENTA LINEA QUE CREA REVERSO PARA ADICIONAR PROCESO QUE BUSCA TIPO
		 * DE DOCUMENTO DE REVERSO Y SE LO PASA COMO PARAMETRO A LA COPIA
		 * INVERSA DE LA FACTURA MInvoice reversal = copyFrom (this,
		 * getDateInvoiced(), getC_DocType_ID(), false, get_Trx(), true);
		 */
		MInOut reversal = null;
		// BUSCA TIPO DE DOCUMENTO PARA REVERSO AVANTED PRIME LOC001
		int RevDoctType = getReversalDoctType(getC_DocType_ID());
		/*
		 * SI NO EXISTE TIPO DE DOCUMENTO PARA REVERSO, LA ANULACION MANTIENE EL
		 * MISMO TIPO DE DOCUMENTO DE LA FACTURA ORIGINAL SI EXISTE COLOCA EN LA
		 * ANULACION EL TIPO DE DOCUMENTO DE REVERSO ENCONTRADO AVANTED PRIME
		 * LOC001
		 */

		if (RevDoctType == 0) {	
			reversal = copyFrom (this, getMovementDate(),
					getC_DocType_ID(), isSOTrx(), dt.isReturnTrx(), false, get_Trx(), true);
		} else {
			reversal = copyFrom (this, getDateAcct(),
					RevDoctType, isSOTrx(), dt.isReturnTrx(), false, get_Trx(), true);			
			                                            // LOC001 Hmora
														// 15/12/2008
														// localización para que
														// tome la fecha
														// contable en el
														// reverso del documento
		}
		/* FIN LOCALIZACION VENEZUELA JGMS 01/12/2007 */
		...
		for (int i = 0; i < rLines.length; i++)
		{
			...
			//2014-01-30 Carlos Parada Subtract Qty Reserver for Original Sales Order
			if (sLines[i].getOrig_OrderLine_ID()!=0 && IsReturnFromOutPutProcess()){
				MOrderLine origLine = new MOrderLine(getCtx(), sLines[i].getOrig_OrderLine_ID(), get_Trx());
				origLine.setQtyReserved(origLine.getQtyReserved().subtract(sLines[i].getQtyEntered()));
				origLine.save(get_Trx());
			}
			//End Carlos Parada
	...
	/*
	 * LOCALIZACION VENEZUELA JGMS 01/12/2006 AVANTED PRIME LOC001 METODO public
	 * int getReversalDoctType(int v_DocType) BUSCA EN LA TABLA C_DOCTYPE CAMPO
	 * CUSTOMIZADO QUE DETERMINA EL TIPO DE DOCUMENTO UTILIZADO PARA REVERSOS
	 * (ANULACIONES)
	 */
	public int getReversalDoctType(int v_DocType) {
		String sql = " SELECT COALESCE(XX_Void_C_DocType_ID, 0) "
				+ " FROM C_DocType d " + " WHERE d.C_DocType_ID=" + v_DocType
				+ " AND   IsActive= 'Y'";
		try {
	...
	/**
	 * Carlos Parada Evaluate if return from OutPut Control Material
	 * @return
	 */
	private boolean IsReturnFromOutPutProcess()
	{
		String sql = " Select M_RMA_ID "
				+ " FROM CUST_RecordWeight rw WHERE rw.M_RMA_ID=" + getC_Order_ID();
