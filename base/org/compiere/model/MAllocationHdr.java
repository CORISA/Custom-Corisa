...
public final class MAllocationHdr extends X_C_AllocationHdr implements DocAction
{
	...
	/**************************************************************************
	 * 	Accrued Allocation. (Modificado por José Chahine)
	 * 	Period needs to be open
	 *	@return true if accrued
	 */
	private boolean accruedIt()
	{
		Timestamp fechaAct = new Timestamp(Env.getCtx().getContextAsTime("#Date"));
		MAllocationLine[] iLines = getLines(true);
		String msg = MPeriod.isOpen(this, iLines, fechaAct, MDocBaseType.DOCBASETYPE_PaymentAllocation);
		if (msg != null)
			throw new IllegalStateException(msg);

		//	Deep Copy
		MAllocationHdr reversal = copyFrom (this, fechaAct, fechaAct,
			get_Trx());
		if (reversal == null)
		{
			m_processMsg = "Could not create Payment Allocation Reversal";
	...
	//FR [ 1866214 ]
		 ...
		 * @throws SQLException 
		 */
		public static MAllocationHdr[] getOfCash (Ctx ctx, int C_Cash_ID, String trxName) 
		{
			...
			final String whereClause = "IsActive='Y'"
				+ " AND EXISTS (SELECT 1 FROM C_CashLine cl, C_AllocationLine al "
					+ "where cl.C_Cash_ID="+C_Cash_ID+" and al.C_CashLine_ID=cl.C_CashLine_ID "
							+ "and C_AllocationHdr.C_AllocationHdr_ID=al.C_AllocationHdr_ID)";
			Query query = MTable.get(Env.getCtx(),Table_ID)
								.createQuery(whereClause);
			ps = DB.prepareStatement("Select * From C_AllocationHdr Where " + query.getWhereClause(), null);
			
			List<MAllocationHdr> list=new ArrayList<MAllocationHdr>();//= query.list();
			try {
				rs = ps.executeQuery();
			
				
				while (rs.next())			
					list.add(new MAllocationHdr(Env.getCtx(), rs, null));
