	...
	implements FormPanel, ActionListener, TableModelListener, ASyncProcess
{
	...
	private void dynInit()
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				//fieldBankAccount.addItem(new String("Hola"));
		...
		try
		{
			...
			while (rs.next())
			{
				...
				//fieldBPartner.addItem(new KeyNamePair(0, ""));
				fieldBPartner.addItem(pp);
			}
			rs.close();
			pstmt.close();
		...
		m_sql = miniTable.prepareTable(new ColumnInfo[] {
			...
			new ColumnInfo(Msg.translate(ctx, "C_BPartner_ID"), "bp.Name", KeyNamePair.class, true, false, "i.C_BPartner_ID"),
			new ColumnInfo(Msg.translate(ctx, "DocumentNo"), "i.DocumentNo", String.class),
			new ColumnInfo(Msg.translate(ctx, "AccountNo"), "bpfirstaccountno(i.C_BPartner_ID)", String.class), //	Yamel Senih 1/09/2012 12:02 Cuenta Bancaria
			new ColumnInfo(Msg.getMsg(ctx, "AmountDue"), "currencyConvert(invoiceOpen(i.C_Invoice_ID,i.C_InvoicePaySchedule_ID),i.C_Currency_ID, ?,?,i.C_ConversionType_ID, i.AD_Client_ID,i.AD_Org_ID)", BigDecimal.class),
			...
			"i.IsSOTrx=? AND IsPaid='N'"
			//	Different Payment Selection
			//Carlos Parada 12/12/2013 Check Active From PaySelect
			+ " AND NOT EXISTS (SELECT * FROM C_PaySelectionLine psl"
			...
			+                 " WHERE i.C_Invoice_ID=psl.C_Invoice_ID AND ps.IsActive='Y'"
			+				  " AND (pmt.DocStatus IS NULL OR pmt.DocStatus NOT IN ('VO','RE')) )"
			+ " AND i.DocStatus IN ('CO','CL')"
			//End Carlos Parada
			//	Yamel Senih 11/09/2012, 11:23 No mostrar los Documentos en Negociación
			+ " AND i.IsInDispute = 'N'"
			//	Fin Yamel Senih
			+ " AND i.AD_Client_ID=?",	//	additional where & order in loadTableInfo()
			true, "i");
	...
	private void loadBankInfo()
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				//fieldPaymentRule.addItem(new Integer(10));
	...
	public void calculateSelection()
	{
		...
		for (int i = 0; i < rows; i++)
		{
			...
			if (id.isSelected())
			{
				//	Yamel Senih 11/09/2012, 11:53
				//	Se Coloca la constante en lugar de (9)
				BigDecimal amt = (BigDecimal)miniTable.getModel().getValueAt(i, PAY_AMT);
	...
	//	Trx p_trx = Trx.get(trx, true);	p_trx needs to be committed too
		...
		//	Yamel Senih 11/09/2012 11:44 Se Optiene el ValueNamePair para obtener el valor y el nombre
		...
		//	
		if (X_C_Order.PAYMENTRULE_DirectDebit.equals(paymentRule.getValue()))
		{
			isSOTrx = true;
		}
		...
		for (int i = 0; i < rows; i++)
		{
			...
			{
				line += 10;
				MPaySelectionLine psl = new MPaySelectionLine (m_ps, line, paymentRule.getValue());
				int C_Invoice_ID = id.getRecord_ID().intValue();
				//	Yamel Senih 11/09/2012 11:48
				//	Se establecen constantes para el manejo de las columnas de Monto Abierto y Monto de Pago
				BigDecimal OpenAmt = (BigDecimal)miniTable.getModel().getValueAt(i, OPEN_AMT);
				BigDecimal PayAmt = (BigDecimal)miniTable.getModel().getValueAt(i, PAY_AMT);
				//	Fin Yamel Senih
