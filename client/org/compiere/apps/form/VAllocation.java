...
//	private int			i_multiplier = 10;
	...
	private void loadBPartner ()
	{
		...
		StringBuffer sql = new StringBuffer("SELECT p.DateTrx,(SELECT '('||name||') ' FROM C_DOCTYPE WHERE C_DOCTYPE_ID = p.C_DOCTYPE_ID)||p.DocumentNo,p.C_Payment_ID,"  //  1..3
...
//				line.add(rs.getBigDecimal(8));		//  6/8-Multiplier
		...
		columnNames.add(Msg.translate(Env.getCtx(), "IsPrepayment")); //AVANTED PRIME 05/11/2009 MEJORAS AL ESTANDAR
...
//		paymentTable.setColumnClass(i++, BigDecimal.class, true);      	//  8-Multiplier
		paymentTable.setColumnClass(i++,String.class, true);       //  9 PREPAYMENT AVANTED PRIME 05/11/2009 MEJORAS AL ESTANDAR
		...
		/** Changed By Jorge Colmenarez 2014-06-02 
		 *  Alter Sql in the clausule "WHERE" on i.docstatus IN ('CO','CL') by i.docstatus NOT IN ('DR','IP','IN') 
		 *  because result not show invoices that is show on the report items open 
		**/
		sql = new StringBuffer("SELECT i.DateInvoiced,(SELECT '('||name||') ' FROM C_DOCTYPE WHERE C_DOCTYPE_ID = i.C_DOCTYPE_ID)||i.DocumentNo,i.C_Invoice_ID," //  1..3 //AVANTED PRIME 05/11/2009 MEJORAS AL ESTANDAR
			...
			*/
			...
			+ "i.MultiplierAP " 
			+ ", isindispute, (Select value||' '||name from C_Project proj where proj.C_Project_ID = i.C_Project_ID) " //AVANTED PRIME LOCALIZACION 
			+ "FROM C_Invoice_v i"		//  corrected for CM/Split
			+ " INNER JOIN C_Currency c ON (i.C_Currency_ID=c.C_Currency_ID) "
//			+ "WHERE (i.IsPaid='N') AND i.Processed='Y'" // AVANTED PRIME LOCALIZACION
			+ "WHERE ((invoiceopen(c_invoice_id,null) <> 0 AND i.docstatus NOT IN ('DR','IP','IN')) or i.isindispute ='Y') AND i.Processed='Y'" // AVANTED PRIME LOCALIZACION
...
//				line.add(rs.getBigDecimal(9));		//	8/10-Multiplier
				...
				//if (Env.ZERO.compareTo(open) != 0)// AVANTED PRIME LOCALIZACION
				if ((Env.ZERO.compareTo(open) != 0)||(rs.getString(10).compareTo("Y")==0)) // AVANTED PRIME LOCALIZACION
					data.add(line);
			}
			rs.close();
			pstmt.close();
